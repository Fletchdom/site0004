...
function imageTag(item) {
  return `<img src="${escapeAttr(item.poster)}" alt="${escapeAttr(item.title)}" loading="eager" decoding="async">`;
}
...
function renderHome() {
  ...
  const hero = hot[0];
  document.getElementById("mainScreen").innerHTML = `<a href="./movie.html?id=${hero.id}">
    ${imageTag(hero)}
    <span><b>${hero.title}</b><em>${hero.kind} / ${hero.score}</em></span>
  </a>`;
  document.getElementById("showtimeList").innerHTML = hot.slice(1, 6).map(featureRow).join("");
  document.getElementById("featureList").innerHTML = hot.slice(6, 13).map(featureRow).join("");
  document.getElementById("rankList").innerHTML = sortScore(items).slice(0, 9).map((item, index) => `<li><a href="./movie.html?id=${item.id}"><span>${String(index + 1).padStart(2, "0")} ${item.title}</span><b>${item.score}</b></a></li>`).join("");
...
function renderLibrary() {
  ...
  document.getElementById("libraryTitle").textContent = kind === "全部" ? "全部真实海报内容" : `${kind}真实海报`;
...
function renderDetail() {
  ...
  document.title = `${item.title}-${item.kind}真实海报资料-日本电影在线`;
  ...
  document.getElementById("detailRoot").innerHTML = `
    ...
    <article class="detail-copy">
      ...
      <p>${item.summary}</p>
      <a class="button gold" href="./library.html?kind=${encodeURIComponent(item.kind)}">查看同频道真实海报</a>
    </article>`;
...
function markBrokenImages() {
  document.querySelectorAll("img").forEach((img) => {
    img.addEventListener("error", () => {
      img.closest(".poster-card,.feature,.screen-card,.detail-poster")?.classList.add("image-missing");
    }, { once: true });
  });
}
